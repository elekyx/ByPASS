...
namespace SSD_Components
{
	Data_Cache_Manager_Flash_Advanced::Data_Cache_Manager_Flash_Advanced(const sim_object_id_type& id, Host_Interface_Base* host_interface, NVM_Firmware* firmware, NVM_PHY_ONFI* flash_controller,
		unsigned long long total_capacity_in_bytes,
		...
		dram_execution_list_turn(0), back_pressure_buffer_max_depth(back_pressure_buffer_max_depth)
	{
		...
		gost = new Data_Cache_Flash*[2];
		for (unsigned int i = 0; i < 2; i++) {
			gost[i] = new Data_Cache_Flash(capacity_in_pages);
			ghost_access_count[i] = 0;
			ghost_hit_count[i] = 0;
		}
		
		per_stream_cache = new Data_Cache_Flash*[stream_count];
		sharedCacheComputer = new Data_Cache_Flash(capacity_in_pages/2);
		sharedCacheSSD = new Data_Cache_Flash(capacity_in_pages/2);

	
		for (unsigned int i = 0; i < Host_Components::StreamCount::HostStreamCount; i++) {
			per_stream_cache[i] = sharedCacheComputer;
		}

		for (unsigned int i = Host_Components::StreamCount::HostStreamCount; i < stream_count; i++) {
			per_stream_cache[i] = sharedCacheSSD;
		}
	...
	unsigned int Data_Cache_Manager_Flash_Advanced::get_gost_id(stream_id_type stream_id) const
	{
		return stream_id >= Host_Components::StreamCount::HostStreamCount ? 1 : 0;
	}


	void Data_Cache_Manager_Flash_Advanced::record_ghost_access(stream_id_type stream_id, LPA_type lpa,
		const data_cache_content_type content, const data_timestamp_type timestamp, const page_status_type sectors_bitmap)
	{
		unsigned int gostId = get_gost_id(stream_id);
		ghost_access_count[gostId]++;

		Data_Cache_Slot_Type* slot = gost[gostId]->Find_slot(stream_id, lpa);
		if (slot != nullptr) {
			ghost_hit_count[gostId]++;
			sim_time_type merged_timestamp = slot->Timestamp;
			NVM::memory_content_type merged_content = slot->Content;
			if (timestamp > merged_timestamp) {
				merged_timestamp = timestamp;
				merged_content = content;
			}
			gost[gostId]->Update_data(slot, stream_id, lpa, merged_content, merged_timestamp,
				sectors_bitmap | slot->State_bitmap_of_existing_sectors);
			return;
		}

		if (!gost[gostId]->Check_free_slot_availability()) {
			gost[gostId]->Evict_one_slot_lru();
		}
		gost[gostId]->Insert_write_data(stream_id, lpa, content, timestamp, sectors_bitmap);
	}

	void Data_Cache_Manager_Flash_Advanced::reset_ghost_window_stats()
	{
		ghost_access_count[0] = 0;
		ghost_access_count[1] = 0;
		ghost_hit_count[0] = 0;
		ghost_hit_count[1] = 0;
	}
	...
	Data_Cache_Manager_Flash_Advanced::~Data_Cache_Manager_Flash_Advanced()
	{
		...
		delete sharedCacheComputer;
		delete sharedCacheSSD;
		delete[] per_stream_cache;

		unsigned int queue_count = shared_dram_request_queue ? 1 : stream_count;
	...
	void Data_Cache_Manager_Flash_Advanced::process_new_user_request(User_Request* user_request)
	{
		...
		unsigned int gostId = 0;
		if(user_request->Stream_id >= Host_Components::StreamCount::HostStreamCount){
			gostId = 1;
		}
			...
			switch (caching_mode_per_input_stream[user_request->Stream_id]) {
				...
				case Caching_Mode::WRITE_READ_CACHE:
				{
					...
					while (it != user_request->Transaction_list.end()) {
						...
						record_ghost_access(tr->Stream_id, tr->LPA, tr->Content, tr->DataTimeStamp, tr->read_sectors_bitmap);

						Data_Cache_Slot_Type* slot_ptr = per_stream_cache[tr->Stream_id]->Find_slot(tr->Stream_id, tr->LPA);
						if (slot_ptr != nullptr) {
							if (tr->Stream_id < Host_Components::StreamCount::HostStreamCount) {
								per_stream_cache[tr->Stream_id]->Mark_slot_accessed(slot_ptr);
							} else {
								per_stream_cache[tr->Stream_id]->Mark_slot_accessed(slot_ptr);
							}
							Data_Cache_Slot_Type& slot = *slot_ptr;
							page_status_type available_sectors_bitmap = slot.State_bitmap_of_existing_sectors & tr->read_sectors_bitmap;
							if (available_sectors_bitmap == tr->read_sectors_bitmap) {
								user_request->Sectors_serviced_from_cache += count_sector_no_from_status_bitmap(tr->read_sectors_bitmap);
									per_stream_cache[tr->Stream_id]->Update_data_read(slot_ptr, tr->Stream_id, tr->LPA, slot.Content,
									slot.Timestamp, slot.State_bitmap_of_existing_sectors);
								NVM_Transaction *toDel = (*it);
		...
		else//This is a write request
		{
			switch (caching_mode_per_input_stream[user_request->Stream_id])
			{
				...
				case Caching_Mode::HOST_CACHE:
				{
					if(skip_cache_or_not(user_request)) {
						user_request->ToBeBypassed = true;
						write_before_bypass(user_request);
						static_cast<FTL*>(nvm_firmware)->Address_Mapping_Unit->Translate_lpa_to_ppa_and_dispatch(user_request->Transaction_list);
					}else{
	...
	double Data_Cache_Manager_Flash_Advanced::get_ghost_hit_ratio_host(){
		if (ghost_access_count[0] == 0) {
			return 0.0;
		}
		return (double)ghost_hit_count[0] / ghost_access_count[0];
	}
	double Data_Cache_Manager_Flash_Advanced::get_ghost_hit_ratio_ssd(){
		if (ghost_access_count[1] == 0) {
			return 0.0;
		}
		return (double)ghost_hit_count[1] / ghost_access_count[1];
	}

	NVM_Transaction_Flash_WR* Data_Cache_Manager_Flash_Advanced::eject_a_node_and_change(stream_id_type streamId, User_Request* user_request){
        
        Data_Cache_Flash* evction = NULL;

		K=((float)(sharedCacheComputer->get_capacity()))/ (float)capacity_in_pages;
		StatsWindow& stats_window = StatsWindow::getInstance();

		if(stats_window.host_cache_ratio < K){ //use host
			evction = sharedCacheComputer;
			if(streamId != 0){ //compssd
				sharedCacheComputer->set_capacity(sharedCacheComputer->get_capacity() - 1);
				sharedCacheSSD->set_capacity(sharedCacheSSD->get_capacity() + 1);
			}
		}else{ //use SSD
			evction = sharedCacheSSD;
			if(streamId == 0){//host
				sharedCacheSSD->set_capacity(sharedCacheSSD->get_capacity() - 1);
				sharedCacheComputer->set_capacity(sharedCacheComputer->get_capacity() + 1);
			}
        ...
        if(evction!= nullptr && !evction->Empty()){
            Data_Cache_Slot_Type evicted_slot = evction->Evict_one_slot_lru();
			
            if (evicted_slot.Status == Cache_Slot_Status::DIRTY_NO_FLASH_WRITEBACK) {
                return new NVM_Transaction_Flash_WR(Transaction_Source_Type::CACHE,
                    evicted_slot.streamid, count_sector_no_from_status_bitmap(evicted_slot.State_bitmap_of_existing_sectors) * SECTOR_SIZE_IN_BYTE,
                    evicted_slot.LPA, user_request, IO_Flow_Priority_Class::URGENT, evicted_slot.Content, evicted_slot.State_bitmap_of_existing_sectors, evicted_slot.Timestamp);
            }
        }
        return NULL;
	...
	bool Data_Cache_Manager_Flash_Advanced::is_write_req_in_cache(User_Request* user_request){
		auto it = user_request->Transaction_list.begin();
		
		while (it != user_request->Transaction_list.end()) {
			NVM_Transaction_Flash_WR* tr = (NVM_Transaction_Flash_WR*)(*it);
			if (per_stream_cache[tr->Stream_id]->Find_slot(tr->Stream_id, tr->LPA) != nullptr) {
				// the logical address already exists in the cache
				return true;
			}
			it++;
		}
    ...
    bool Data_Cache_Manager_Flash_Advanced::skip_cache_or_not(User_Request* user_request)
	{
		if (is_write_req_in_cache(user_request)) {
			return false;
		} 

		StatsWindow& stats_window = StatsWindow::getInstance();
		if (stats_window.host_cache_ratio == 0.0)
		{
			return true;
		}
	...
	void Data_Cache_Manager_Flash_Advanced::write_before_bypass(User_Request* user_request)
	{
		auto it = user_request->Transaction_list.begin();
		...
		while (it != user_request->Transaction_list.end()) {
			NVM_Transaction_Flash_WR* tr = (NVM_Transaction_Flash_WR*)(*it);
			record_ghost_access(tr->Stream_id, tr->LPA, tr->Content, tr->DataTimeStamp, tr->write_sectors_bitmap);
			++it;
		}
	}
	...
	void Data_Cache_Manager_Flash_Advanced::write_to_destage_buffer(User_Request* user_request)
	{
		...
		unsigned int gostId = get_gost_id(user_request->Stream_id);	
		...
		while (it != user_request->Transaction_list.end() 
			...
			if (slot_ptr != nullptr) {				
				...
				* This situation may result from out-of-order transaction execution*/
				sim_time_type timestamp = slot_ptr->Timestamp;
				NVM::memory_content_type content = slot_ptr->Content;
				if (tr->DataTimeStamp > timestamp) {
					timestamp = tr->DataTimeStamp;
					content = tr->Content;
				}
				per_stream_cache[tr->Stream_id]->Update_data(slot_ptr, tr->Stream_id, tr->LPA, content, timestamp, tr->write_sectors_bitmap | slot_ptr->State_bitmap_of_existing_sectors);
				...
				if (!per_stream_cache[tr->Stream_id]->Check_free_slot_availability()) {
					...
					// }
					NVM_Transaction_Flash_WR* evction = eject_a_node_and_change(gostId, user_request);
					if(evction){
						evicted_cache_slots->push_back(evction);
						cache_eviction_read_size_in_sectors += count_sector_no_from_status_bitmap(evction->write_sectors_bitmap);
					}
				}
	...
	void Data_Cache_Manager_Flash_Advanced::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction)
	{
		...
		unsigned int gostId = 0;
		if(transaction->Stream_id >= Host_Components::StreamCount::HostStreamCount){
			gostId = 1;
		}
		...
		*  An update read transaction is issued in Address Mapping Unit, but is consumed in data cache manager.*/
		if (transaction->Type == Transaction_Type::READ) {
			...
			switch (Data_Cache_Manager_Flash_Advanced::caching_mode_per_input_stream[transaction->Stream_id])
			{
				...
				case Caching_Mode::WRITE_READ_CACHE:
				{					
					Data_Cache_Slot_Type* slot_ptr = ((Data_Cache_Manager_Flash_Advanced*)_my_instance)->per_stream_cache[transaction->Stream_id]->Find_slot(transaction->Stream_id, transaction->LPA);
					if (slot_ptr != nullptr) {
						// if (transaction->Stream_id < Host_Components::StreamCount::HostStreamCount) {
						// 	((Data_Cache_Manager_Flash_Advanced*)_my_instance)->per_stream_cache[transaction->Stream_id]->Mark_slot_accessed(slot_ptr);
						// } else {
						// 	((Data_Cache_Manager_Flash_Advanced*)_my_instance)->per_stream_cache[transaction->Stream_id]->Mark_slot_accessed(slot_ptr);
						// }
						/*MQSim should get rid of writting stale data to the cache.
						* This situation may result from out-of-order transaction execution*/
						...
						((Data_Cache_Manager_Flash_Advanced*)_my_instance)->per_stream_cache[transaction->Stream_id]->Update_data_read(slot_ptr, transaction->Stream_id, transaction->LPA, content,
							timestamp, ((NVM_Transaction_Flash_RD*)transaction)->read_sectors_bitmap | slot_ptr->State_bitmap_of_existing_sectors);
					} else  {
						if (!((Data_Cache_Manager_Flash_Advanced*)_my_instance)->per_stream_cache[transaction->Stream_id]->Check_free_slot_availability()) {
							...
							// }
							NVM_Transaction_Flash_WR* evction = ((Data_Cache_Manager_Flash_Advanced*)_my_instance)->eject_a_node_and_change(gostId, NULL);
							// Data_Cache_Slot_Type evicted_slot = ((Data_Cache_Manager_Flash_Advanced*)_my_instance)->per_stream_cache[transaction->Stream_id]->Evict_one_slot_lru();
							// if (evicted_slot.Status == Cache_Slot_Status::DIRTY_NO_FLASH_WRITEBACK) {
							if(evction){
								std::list<NVM_Transaction*>* evicted_cache_slots = new std::list<NVM_Transaction*>;
								Memory_Transfer_Info* transfer_info = new Memory_Transfer_Info;
								transfer_info->Size_in_bytes = count_sector_no_from_status_bitmap(evction->write_sectors_bitmap) * SECTOR_SIZE_IN_BYTE;
								evicted_cache_slots->push_back(evction);
								transfer_info->Related_request = evicted_cache_slots;
								transfer_info->next_event_type = Data_Cache_Simulation_Event_Type::MEMORY_READ_FOR_CACHE_EVICTION_FINISHED;
								transfer_info->Stream_id = transaction->Stream_id;
								// unsigned int cache_eviction_read_size_in_sectors = count_sector_no_from_status_bitmap(evction->write_sectors_bitmap);
								// transaction->UserIORequest->Transaction_list.push_back(evction);
								int sharing_id = transaction->Stream_id;
								if (((Data_Cache_Manager_Flash_Advanced*)_my_instance)->shared_dram_request_queue) {
									sharing_id = 0;
								}
								((Data_Cache_Manager_Flash_Advanced*)_my_instance)->back_pressure_buffer_depth[sharing_id] +=
									count_sector_no_from_status_bitmap(evction->write_sectors_bitmap);
			...
			switch (Data_Cache_Manager_Flash_Advanced::caching_mode_per_input_stream[transaction->Stream_id])
			{
				...
				case Caching_Mode::WRITE_READ_CACHE:
				{
					...
					// ((Data_Cache_Manager_Flash_Advanced*)_my_instance)->back_pressure_buffer_depth[sharing_id] -= 1;

					Data_Cache_Slot_Type* slot_ptr = ((Data_Cache_Manager_Flash_Advanced*)_my_instance)->per_stream_cache[transaction->Stream_id]->Find_slot(transaction->Stream_id, ((NVM_Transaction_Flash_WR*)transaction)->LPA);
					if (slot_ptr != nullptr) {
						sim_time_type timestamp = slot_ptr->Timestamp;
						NVM::memory_content_type content = slot_ptr->Content;
