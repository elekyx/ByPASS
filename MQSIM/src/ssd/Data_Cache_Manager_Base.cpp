...
namespace SSD_Components
{
		...
		sharing_mode(sharing_mode), stream_count(stream_count)
	{
		...
		for (unsigned int i = 0; i < stream_count; i++) {
			...
			if(this->caching_mode_per_input_stream[i] == Caching_Mode::HOST_CACHE){
				Host_Components::StreamCount::HostStreamCount++;
			}
		}
	...
	{
		nvm_transaction->STAT_ResponseTime = Simulator->Time();


		StatsWindow& stats_window = StatsWindow::getInstance();
		// stats_window.update_window_stats(nvm_transaction,this);
		stats_window.update_window_stats(nvm_transaction,static_cast<FTL*>(nvm_firmware)->Data_cache_manager);
