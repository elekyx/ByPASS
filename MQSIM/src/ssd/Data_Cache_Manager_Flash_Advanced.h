...
namespace SSD_Components
{
	...
	*/
	...
	protected:
		...
		unsigned long long capacity_in_bytes, capacity_in_pages;
		unsigned int sector_no_per_page;
		Data_Cache_Flash** per_stream_cache;
		Data_Cache_Flash** gost;
		unsigned long long ghost_access_count[2];
		unsigned long long ghost_hit_count[2];
		bool memory_channel_is_busy;
		unsigned long long arrivedReq = 0;
		Data_Cache_Flash* sharedCacheComputer, *sharedCacheSSD;
		...
		NVM_Transaction_Flash_WR* eject_a_node_and_change(stream_id_type streamId, User_Request* user_request);
		bool skip_cache_or_not(User_Request* user_request);
		bool is_write_req_in_cache(User_Request* user_request);
		void write_before_bypass(User_Request* user_request);
		unsigned int get_gost_id(stream_id_type stream_id) const;
		void record_ghost_access(stream_id_type stream_id, LPA_type lpa, const data_cache_content_type content,
			const data_timestamp_type timestamp, const page_status_type sectors_bitmap);
		float K = 0.5f;
		double get_ghost_hit_ratio_host();
		double get_ghost_hit_ratio_ssd();
	};
}
